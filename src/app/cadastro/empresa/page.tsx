    ...
    if (formData.Senha === formData.ConfirmSenha) {
      try {
        ...
        if (response.data) {
          ...
          try {
            const response = await api.post("/api/v1/empresas/login", { Email, Senha },
            {
              headers: {
                // Adicione aqui o cabeçalho personalizado para a empresa
                "authorization": "Empcd",
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen">
      <div className="w-full md:w-1/2 bg-0D9488 md:block hidden">
        <Image className="w-full h-full" src={Logo} alt="Logo Empcd" priority />
      </div>
      <div className="w-full md:w-1/2">
        <div className="p-4 md:p-10 bg-white">
          ...
          <form className="grid grid-cols-2 gap-4" onSubmit={enviarForm}>
            ...
            <div className="mb-4">
              <label htmlFor="ConfirmSenha" className="text-0C5E58 block mb-1">
                Confirme sua senha:
              </label>
              <input
                type="password"
